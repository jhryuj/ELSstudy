...
function varargout = slice_overlay_bai_1(action, varargin);
...
for i = 1:nslices
  ...
  % Xiaoxiao added to convert right-left to left-right of figure;
  bai_img = zeros(size(img, 1), size(img, 2), size(img, 3));
  for ii = 1:size(img, 1)
      for jj = 1:size(img, 2)
          bai_img(ii, size(img, 2)-jj+1, :) = img(ii, jj, :);
      end
  end
  img = bai_img;
